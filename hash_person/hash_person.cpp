...
struct AddressHasher {
  ...
  size_t operator() (const Address& a) const {
    size_t city = strhash(a.city);
    size_t street = strhash(a.street);
    size_t building = inthash(a.building);

    size_t result = city * 1'000'000;
    result += street;
    result *= 1'000'000;
    result += building;
...
struct PersonHasher {
  ...
  size_t operator() (const Person& p) const {
    size_t name = nhash(p.name);
    size_t height = hhash(p.height);
    size_t weight = whash(p.weight);
    size_t address = ahash(p.address);
    // ax^2 + Bx + 2C + D
    size_t x = 3'234'748;
    return (name * x * x + height * x + 2 * weight + address);
  }
